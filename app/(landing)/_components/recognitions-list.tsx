...
// Define recognition items with consistent width
...
export function RecognitionsList() {
  return (
    <div className="relative w-full py-8 bg-white shadow-[0px_0px_40.1px_#00000029]">
      <h3 className="text-center font-semibold text-[#1e1e1e] text-[32px] tracking-[-0.70px] leading-[38.4px] mb-8">
      ...
      <div className="relative w-full overflow-hidden hidden md:block">
        <style jsx>{`
          @keyframes scroll {
            ...
            100% { transform: translateX(-50%); } /* scroll half of duplicated array */
          }
          .animate-scroll {
            display: flex;
        ...
        <div className="animate-scroll">
          {[...recognitionImages, ...recognitionImages].map((img, index) => (
            <div key={index} className="flex items-center justify-center shrink-0 mx-4">
              <Image
                src={img.src}
                alt={img.alt}
                width={img.width}
